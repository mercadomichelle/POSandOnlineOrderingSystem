...
$branch_id = isset($_SESSION['selected_branch']) ? $_SESSION['selected_branch'] : null; // Get the selected branch from session
...
// If branch_id is provided (from session), add the condition; if not, ignore it to fetch all branches
$branch_condition = $branch_id ? "AND oi.branch_id = ?" : "";
...
$sql = "
    ...
    WHERE o.order_source IN ('in-store', 'online') 
    $branch_condition
    $timeframe_condition
...
// Bind parameters only if branch_id is provided
if ($branch_id) {
